...
class HGImageCollectionViewController: UIViewController {
    ...
    var type:Int = 0//0相片 1视频
    
    //照片选择完毕后的回调
    var completeHandler:((_ assets:[String])->())?
    var isClickedComplete=false
    ...
    @objc func finishSelect(){
        if isClickedComplete==false {
            ...
            var ar=[String]()
            //保存到本地
            ar = saveToLocal(assets: assets)

            //调用回调函数
            self.navigationController?.dismiss(animated: true, completion: {
                self.isClickedComplete=false
                self.completeHandler?(ar)
            })
        }
    }
    
    //保存到本地
    func saveToLocal(assets:Array<PHAsset>) -> [String]{
        var array=[String]()
        if self.type==0{
            ...
            for asset in assets {
                ...
                manager.requestImageData(for: asset, options: initialRequestOptions, resultHandler: { (data, dataUI, orientation, info) in
                    //保存在本地
                    let file=NSTemporaryDirectory()
                    let hash=asset.hash
                    let path:String = "\(file)\(hash).png"
                    print(path)
                    let isSave=FileManager.default.createFile(atPath:path, contents: data, attributes: nil)
                    if isSave == true{
                        print("保存成功")
                        array.append(path)
                    }
            ...
            for asset in assets {
                print(asset)
                let manager = PHImageManager.default()
                manager.requestAVAsset(forVideo: asset, options: options, resultHandler: {(avurlAsset,audioMix,dict) in
                    if let avurlAsset = avurlAsset as? AVURLAsset{
                        let videoData = NSData(contentsOf: avurlAsset.url)
                        let file=NSTemporaryDirectory()
                        let hash=asset.hash
                        let path:String = "\(file)\(hash).mov"
