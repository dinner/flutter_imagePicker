...
open class HGImageCollectionViewCell: UICollectionViewCell {
    ...
    open override var isSelected: Bool {
        didSet{
            let podBundle=Bundle.init(for: HGImagePickerController.self)
            if isSelected {
                selectedIcon.image = UIImage(named: "CellBlueSelected.png" ,in: podBundle ,compatibleWith: nil)
            }else{
                selectedIcon.image = UIImage(named: "CellGreySelected.png" ,in:podBundle, compatibleWith:nil)
            }
        }
    }
